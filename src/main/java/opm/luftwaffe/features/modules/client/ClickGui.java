...
public class ClickGui
    ...
    public Setting<String> command = register(new Setting("Command", "luftwaffe.xyz"));
    ...
    public void onSettingChange(ClientEvent event) {
        if (event.getStage() == 2 && event.getSetting().getFeature().equals(this)) {
            if (event.getSetting().equals(this.prefix)) {
                Luftwaffe.commandManager.setPrefix(this.prefix.getPlannedValue());
                Command.sendMessage("Prefix set to " + ChatFormatting.DARK_GRAY + Luftwaffe.commandManager.getPrefix());
            }
            Luftwaffe.colorManager.setColor(this.red.getPlannedValue(), this.green.getPlannedValue(), this.blue.getPlannedValue(), this.hoverAlpha.getPlannedValue());
        }
    }
    ...
    @Override
    public void onLoad() {
        Luftwaffe.colorManager.setColor(this.red.getValue(), this.green.getValue(), this.blue.getValue(), this.hoverAlpha.getValue());
        Luftwaffe.commandManager.setPrefix(this.prefix.getValue());
    }
