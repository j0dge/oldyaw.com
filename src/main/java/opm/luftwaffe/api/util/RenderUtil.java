...
public class RenderUtil
    ...
    public static void drawText(BlockPos pos, String text) {
        ...
        GlStateManager.disableDepth();
        GlStateManager.translate(-((double) Luftwaffe.textManager.getStringWidth(text) / 2.0), 0.0, 0.0);
        Luftwaffe.textManager.drawStringWithShadow(text, 0.0f, 0.0f, -5592406);
        GlStateManager.popMatrix();
    }
    ...
    public static void drawGradientRect(float x, float y, float w, float h, long color, long color2, boolean vertical) {
        ...
        bufferbuilder.begin(7, DefaultVertexFormats.POSITION_COLOR);
        if (vertical) {
            bufferbuilder.pos(x, y + h, 0.0).color(red2, green2, blue2, alpha2).endVertex();
            bufferbuilder.pos(x + w, y + h, 0.0).color(red2, green2, blue2, alpha2).endVertex();
            bufferbuilder.pos(x + w, y, 0.0).color(red, green, blue, alpha).endVertex();
            bufferbuilder.pos(x, y, 0.0).color(red, green, blue, alpha).endVertex();
        } else {
            bufferbuilder.pos(x + w, y + h, 0.0).color(red2, green2, blue2, alpha2).endVertex();
            bufferbuilder.pos(x + w, y, 0.0).color(red2, green2, blue2, alpha2).endVertex();
            bufferbuilder.pos(x, y, 0.0).color(red, green, blue, alpha).endVertex();
    ...
    public static void scissor(float x, float y, float width, float height) {
        ...
        int scissorY = (int)((sr.getScaledHeight() - (y + height)) * scale);
    ...
    public static void drawOutlineCircle(float x, float y, float radius, int segments, int color) {
        float alpha = (color >> 24 & 0xFF) / 255.0F;
        float red = (color >> 16 & 0xFF) / 255.0F;
        float green = (color >> 8 & 0xFF) / 255.0F;
        float blue = (color & 0xFF) / 255.0F;
