...
import opm.luftwaffe.api.event.events.UpdateWalkingPlayerEvent;
import opm.luftwaffe.features.modules.Module;
import opm.luftwaffe.features.setting.Setting;
import opm.luftwaffe.api.util.DamageUtil;
...
public class Killaura extends Module {
    ...
    private void doKillaura() {
        ...
        int wait = !this.delay.getValue().booleanValue() ? 0 : (int) (DamageUtil.getCooldownByWeapon(mc.player) * (this.tps.getValue().booleanValue() ? Luftwaffe.serverManager.getTpsFactor() : 1.0F));
        ...
        if (target == null)
            return;
        if (this.rotate.getValue().booleanValue())
            Luftwaffe.rotationManager.lookAtEntity(target);
