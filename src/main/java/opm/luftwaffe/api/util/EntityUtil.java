...
public class EntityUtil
    ...
    public static boolean isntValid(Entity entity, double range) {
        return entity == null || EntityUtil.isDead(entity) || entity.equals(EntityUtil.mc.player) || entity instanceof EntityPlayer && Luftwaffe.friendManager.isFriend(entity.getName()) || EntityUtil.mc.player.getDistanceSq(entity) > MathUtil.square(range);
    }
    ...
    public static Color getColor(Entity entity, int red, int green, int blue, int alpha, boolean colorFriends) {
        ...
        if (entity instanceof EntityPlayer && colorFriends && Luftwaffe.friendManager.isFriend((EntityPlayer) entity)) {
    ...
    public static Map<String, Integer> getTextRadarPlayers() {
        ...
        for (EntityPlayer player : EntityUtil.mc.world.playerEntities) {
            ...
            output.put(healthSB.toString() + " " + (Luftwaffe.friendManager.isFriend(player) ? ChatFormatting.AQUA : ChatFormatting.RED) + player.getName() + " " + distanceSB.toString() + " \u00c2\u00a7f0", (int) EntityUtil.mc.player.getDistance(player));
    ...
    public static double getStrictBaseSpeed(Double baseSpeed) {
        double maxModifier = baseSpeed;
        if (mc.player.isPotionActive(Objects.requireNonNull(Potion.getPotionById((int)1)))) {
            maxModifier *= 1.0 + 0.1 * (double)(Objects.requireNonNull(mc.player.getActivePotionEffect(Objects.requireNonNull(Potion.getPotionById((int)1)))).getAmplifier() + 1);
        }
        return maxModifier;
    }
    ...
    public static void phaseSpeed(MoveEvent event, double speed) {
        if (isMoving()) {
            float boost = 0.0f;
            if (!mc.player.collidedHorizontally && PhaseWalk.INSTANCE.isPhasing() && mc.player.onGround) {
                boost = random.nextFloat() * 0.2f + 0.1f;
            }
            double[] strafe = strafe(speed + (double)boost);
            event.setX(strafe[0]);
            event.setZ(strafe[1]);
        } else {
            event.setX(0.0);
