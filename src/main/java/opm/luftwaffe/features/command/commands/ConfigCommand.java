...
public class ConfigCommand extends Command {
    ...
    public void execute(String[] commands) {
        ...
        if (commands.length >= 3) {
            switch (commands[0]) {
                case "save":
                    Luftwaffe.configManager.saveConfig(commands[1]);
                    sendMessage(ChatFormatting.GREEN + "Config '" + commands[1] + "' has been saved.");
                    return;
                case "load":
                    if (Luftwaffe.configManager.configExists(commands[1])) {
                        Luftwaffe.configManager.loadConfig(commands[1]);
