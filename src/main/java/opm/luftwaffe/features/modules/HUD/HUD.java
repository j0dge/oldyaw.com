...
import opm.luftwaffe.api.event.events.ClientEvent;
...
public class HUD extends Module {
    ...
    private final Setting<Boolean> grayNess = (new Setting("Gray", Boolean.valueOf(true)));
    private final Setting<Boolean> renderingUp = (new Setting("RenderingUp", Boolean.valueOf(false), "Orientation of the HUD-Elements."));
    private final Setting<Boolean> waterMark = (new Setting("Watermark", Boolean.valueOf(false), "displays watermark"));
    private final Setting<Boolean> arrayList = (new Setting("ActiveModules", Boolean.valueOf(false), "Lists the active modules."));
    ...
    private final Setting<Boolean> armor = (new Setting("Armor", Boolean.valueOf(false), "ArmorHUD"));
    private final Setting<Boolean> totems = (new Setting("Totems", Boolean.valueOf(false), "TotemHUD"));
    private final Setting<Boolean> greeter = (new Setting("Welcomer", Boolean.valueOf(false), "The time"));
    ...
    public Setting<String> command = (new Setting("Command", "luftwaffe"));
    public Setting<TextUtil.Color> commandColor = (new Setting("NameColor", TextUtil.Color.BLUE));;
    public Setting<Boolean> friendMsg = register(new Setting("Friend Message", Boolean.valueOf(true), "notify nigga if ur adding him"));
    public Setting<Boolean> magenDavid = (new Setting("MagenDavid", Boolean.valueOf(false), "draws magen david"));
    public Setting<Integer> animationHorizontalTime = (new Setting("AnimationHTime", Integer.valueOf(500), Integer.valueOf(1), Integer.valueOf(1000), v -> this.arrayList.getValue().booleanValue()));
    public Setting<Integer> animationVerticalTime = (new Setting("AnimationVTime", Integer.valueOf(50), Integer.valueOf(1), Integer.valueOf(500), v -> this.arrayList.getValue().booleanValue()));
    public Setting<RenderingMode> renderingMode = (new Setting("Ordering", RenderingMode.ABC));
    public Setting<Integer> waterMarkY = (new Setting("WatermarkPosY", Integer.valueOf(2), Integer.valueOf(0), Integer.valueOf(20), v -> this.waterMark.getValue().booleanValue()));
    public Setting<Boolean> time = (new Setting("Time", Boolean.valueOf(false), "The time"));
    public Setting<Integer> lagTime = (new Setting("LagTime", Integer.valueOf(1000), Integer.valueOf(0), Integer.valueOf(2000)));
    private int color;
    private boolean shouldIncrement;
    private int hitMarkerTimer;

    public HUD() {
        super("Metrics", "based information", Category.HUD, true, false, false);
        setInstance();
    }
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        if (this.waterMark.getValue().booleanValue()) {
            String string = this.command.getPlannedValue() + Luftwaffe.MODVER;
        ...
        if (this.arrayList.getValue().booleanValue())
            if (this.renderingUp.getValue().booleanValue()) {
                if (this.renderingMode.getValue() == RenderingMode.ABC) {
                    for (int k = 0; k < Luftwaffe.moduleManager.sortedModulesABC.size(); k++) {
                        String str = Luftwaffe.moduleManager.sortedModulesABC.get(k);
                    ...
                    for (int k = 0; k < Luftwaffe.moduleManager.sortedModules.size(); k++) {
                        Module module = Luftwaffe.moduleManager.sortedModules.get(k);
        ...
        if (this.renderingUp.getValue().booleanValue()) {
            if (this.potions.getValue().booleanValue()) {
                ...
                for (PotionEffect potionEffect : effects) {
                    String str = Luftwaffe.potionManager.getColoredPotionString(potionEffect);
                    i += 10;
            ...
            if (this.speed.getValue().booleanValue()) {
                String str = grayString + "Speed " + ChatFormatting.WHITE + Luftwaffe.speedManager.getSpeedKpH() + " km/h";
                i += 10;
            ...
            if (this.tps.getValue().booleanValue()) {
                String str = grayString + "TPS " + ChatFormatting.WHITE + Luftwaffe.serverManager.getTPS();
                i += 10;
                this.renderer.drawString(str, (width - this.renderer.getStringWidth(str) - 2), (height - 2 - i), (ClickGui.getInstance()).rainbow.getValue().booleanValue() ? (((ClickGui.getInstance()).rainbowModeA.getValue() == ClickGui.rainbowModeArray.Up) ? ColorUtil.rainbow(counter1[0] * (ClickGui.getInstance()).rainbowHue.getValue().intValue()).getRGB() : ColorUtil.rainbow((ClickGui.getInstance()).rainbowHue.getValue().intValue()).getRGB()) : this.color, true);
                counter1[0] = counter1[0] + 1;
            }
            String fpsText = grayString + "FPS " + ChatFormatting.WHITE + Minecraft.debugFPS;
            String str1 = grayString + "Ping " + ChatFormatting.WHITE + Luftwaffe.serverManager.getPing();
        ...
        String direction = this.direction.getValue().booleanValue() ? Luftwaffe.rotationManager.getDirection4D(false) : "";
    ...
    public void renderLag() {
        int width = this.renderer.scaledWidth;
        if (Luftwaffe.serverManager.isServerNotResponding()) {
            String text = ChatFormatting.RED + "Server not responding " + MathUtil.round((float) Luftwaffe.serverManager.serverRespondingTime() / 1000.0F, 1) + "s.";
