...
public class PlayerUtil implements Util {
    ...
    public static BlockPos getPlayerPos() {
        double decimalPoint = PlayerUtil.mc.player.posY - Math.floor(PlayerUtil.mc.player.posY);
        return new BlockPos(PlayerUtil.mc.player.posX, decimalPoint > 0.8 ? Math.floor(PlayerUtil.mc.player.posY) + 1.0 : Math.floor(PlayerUtil.mc.player.posY), PlayerUtil.mc.player.posZ);
    }

    public static double[] forwardYaw(double speed, float yaw) {
        float forward = 1.0f;
        float side = 0.0f;
